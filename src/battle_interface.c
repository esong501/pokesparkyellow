...
void TryAddPokeballIconToHealthbox(u8 healthboxSpriteId, bool8 noStatus)
{
    ...
    if (gBattleTypeFlags & (BATTLE_TYPE_FIRST_BATTLE | BATTLE_TYPE_OLD_MAN_TUTORIAL | BATTLE_TYPE_PIKACHU | BATTLE_TYPE_POKEDUDE))
        return;
...
s32 MoveBattleBar(u8 battlerId, u8 healthboxSpriteId, u8 whichBar, u8 unused)
{
    ...
    if (whichBar == HEALTH_BAR)
    {
        ...
        currentBarValue = CalcNewBarValue(gBattleSpritesDataPtr->battleBars[battlerId].maxValue,
                                          ...
                                          B_HEALTHBAR_NUM_TILES, max(gBattleSpritesDataPtr->battleBars[battlerId].maxValue / 32, 1));
    }
    else // exp bar
    {
