...
struct PartyMenuInternal
{
    ...
    u32 messageId:14;
    u8 windowId[3];
    // changing to add nickname and change moves
    u8 actions[10];
    u8 numActions;
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (GetNumberOfRelearnableMoves(&mons[slotId]) != 0)
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, CURSOR_OPTION_CHANGEMOVES);
...
static void Task_PartyMenuReplaceMove(u8 taskId)
{
    struct Pokemon *mon;
    u16 move;
    // tracking PP to not replenish it
    u8 oldPP;
